...
def build_data(settings, data_directory):
    ...
    if os.path.exists(data_savename) is False:
        ...
        analog_input, analog_output, input_standard_dict, output_standard_dict, __ = process_input_output(
        ...
        soi_train_input, soi_train_output, __, __, __, = process_input_output(
...
def extract_region(data, region=None, lat=None, lon=None, mask_builder = 0):
    if region is None:
    ...
    if mask_builder:
        ...
        weighting = np.sqrt(np.cos(np.deg2rad(ilat)))
